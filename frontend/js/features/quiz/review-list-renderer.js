...
import { hasAnswer, isTextInputType } from '../../core/grading.js';
...
export class ReviewListRenderer {
    ...
    render({ questions, answers, filterMode, totalCount }) {
        ...
        questions.forEach((q, i) => {
            ...
            if (status === FILTER_MODES.WRONG) card.classList.add('is-wrong');
            else if (status === FILTER_MODES.CORRECT) card.classList.add('is-correct');
            else card.classList.add('is-unanswered');
    ...
    _buildCardHtml(q, i, st, status, totalCount, ptsPerQ) {
        ...
        return (
            ...
            `<div class="q-badge-num">${i + 1}/${totalCount}</div></div>` +
